...
void sdlJeu::drawTerrain(){
    ...
    //Il faut rajouter à la position du joueur la taille de son sprite/2 pour obtenir sa position effective
    jeu.getTerrain()->terrainBody->ApplyForceToCenter(b2Vec2(-playerPos.x+SPRITE_SIZE/2, 0),1);
...
void sdlJeu::sdlBoucle () {
	...
	while (!quit) {
        ...
        bool finished = jeu.isLevelFinished();
        
        //GESTION LEVELS / COLLISIONS
        if(finished /*&& !hasLost*/){
                    currentLevel++;
                    setTimer(startTime-=20);
                    jeu.getPlayer()->initPlayer(jeu.world);
                    jeu.getPlayer()->setPosition(b2Vec2(100,500),0);

                    jeu.getTerrain()->tabHillPoints.clear();
                    jeu.getTerrain()->generateHillPoints(++currentLevel,.001,2);
                    for (b2Fixture* f = jeu.getTerrain()->terrainBody->GetFixtureList(); f; f = f->GetNext())
                    {
                        jeu.getTerrain()->terrainBody->DestroyFixture(f);
                    }

                    jeu.getTerrain()->initTerrain(jeu.world);
        }

        // if(!jeu.getPlayer()->isInAir) std::cout << "impact" << std::flush;
